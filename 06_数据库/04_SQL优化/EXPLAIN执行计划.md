...
MySQL 优化器的核心是一个 **Cost-Based Optimizer（基于代价的优化器）**。它为每条 SQL 生成多个可能的执行计划，计算每个计划的代价，选择代价最低的。

```
代价 = CPU 代价 + IO 代价

CPU 代价：比较行数（rows） × 条件评估成本
IO 代价：
  - 索引扫描：需要读取的索引页数
  - 全表扫描：表的聚簇索引页数（clustered_index_pages）
  - 回表：随机 IO 次数 ≈ 预计扫描行数
...
-- 关键字段：
-- n_rows：估算的行数（非精确值）
-- clustered_index_size：聚簇索引占用的页数
```

统计信息通过以下方式更新：
- **自动更新**：表中数据变更超过 10%（）
- **手动更新**：
- **持久化统计信息**（MySQL 8.0 默认）：存在  中，重启不丢失

### Index Merge：为什么 OR 有时也能走索引

MySQL 5.0+ 支持 **Index Merge**——对多个索引分别扫描，再合并结果：

```
Index Merge 的两种策略：
1. Sort-Union：分别从 idx_a 和 idx_b 取出 row_id → 排序 → 去重 → 回表
2. Intersect：分别从 idx_a 和 idx_b 取出 row_id → 取交集 → 回表

触发条件：WHERE a = 1 OR b = 2（Sort-Union）
         WHERE a = 1 AND b = 2（Intersect，且两个条件各选一个索引）

查看是否用了 Index Merge：EXPLAIN 的 Extra 字段
  → Using union(idx_a,idx_b)   ← Sort-Union
  → Using intersect(idx_a,idx_b) ← Intersect
```
...
2. EXPLAIN 看执行计划
   ...
   → key 是否为 NULL（没用索引）
   → rows 是否过大
   → Extra 是否有 filesort / Using temporary
3. 根据问题定位优化方向
   → ALL / key=NULL → 考虑加索引
   → filesort → 考虑覆盖索引或调整 ORDER BY
   → Using temporary → 考虑 GROUP BY 列加索引
4. 优化后再次 EXPLAIN 验证
```

### EXPLAIN vs EXPLAIN ANALYZE（MySQL 8.0+）
...
-- 输出包含 actual_time、actual_rows，比 EXPLAIN 更准确
-- 注意：会真正执行 SQL，生产环境慎用
```
