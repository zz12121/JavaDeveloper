...
> AQS（AbstractQueuedSynchronizer）是 JUC 的骨架——ReentrantLock、CountDownLatch、Semaphore、ReentrantReadWriteLock 全部基于它。理解 AQS，才能读懂所有 JUC 锁的源码。

---

## 这个问题为什么存在？

Java 没有提供「通用的锁」让开发者直接用，只提供了 `synchronized` 和 `wait/notify`。但 `synchronized` 不够灵活——不能中断、不能超时、不能尝试获取、不能多个条件变量。
...
线程调用 lock()
  │
  ├─ tryAcquire(1) → 成功 → 设置 exclusiveOwnerThread = 当前线程 → 返回
  │
  └─ tryAcquire(1) → 失败
       │
       ├─ addWaiter()：把线程包装成 Node，CAS 追加到队尾
       │
...
> 在并发场景下，`node.next` 可能还没被设置（指向 null），但 `prev` 指针一定已经设置好了。
...
`ReentrantLock` 的 `newCondition()` 返回的就是 AQS 内部的 `ConditionObject`：
...
```
AQS 队列（获取锁的队列）        Condition 队列（等待条件的队列）
┌─────┐                        ┌─────┐
│head │                        │first│
├─────┤                        ├─────┤
│node1│                        │node2│  ← 调用 await 的线程
└─────┘                        └─────┘
     ↑                                │
     └────────────────────────────────┘
           signal() 时被转移到 AQS 队列
...
共享模式（如 `Semaphore`、`CountDownLatch`）有一个独占模式没有的优化：

```java
// 共享模式获取成功时，会主动唤醒后继的共享节点
if (tryAcquireShared(arg) >= 0) {
    doAcquireShared(arg);
    // 如果还有剩余资源，主动唤醒后继共享节点
    if (propagate)
        doReleaseShared();  // 传播式唤醒
}
```

> 这个优化叫 **「共享传播」**（propagate），防止「信号丢失」。
> 如果不传播，最后一个释放的线程如果没调用 `releaseShared()`，
> 等待队列中的线程可能永远不醒。
...
> 虽然 `Condition.await()` 在 Java 层面已处理，但**规范上仍要求用 while 循环**。
...
### 4. 自定义 AQS 子类时 tryAcquire 必须用 CAS
