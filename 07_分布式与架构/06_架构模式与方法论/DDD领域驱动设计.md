...
DDD不是"银弹"，而是**复杂业务领域的建模方法论**。

核心价值：
1. **提供微服务拆分依据**（限界上下文 = 微服务边界）
2. **业务语义清晰**（代码即文档，新人能看懂）
3. **应对复杂业务变化**（业务规则封装在领域对象，变化不影响应用层）

但DDD有学习成本，**简单CRUD没必要上DDD**。渐进式演进：先简单，业务复杂了再引入。

**面试最高频追问**：
1. "DDD的核心概念？" → 限界上下文、聚合根、实体、值对象、领域事件
2. "怎么划分微服务？" → 事件风暴（Event Storming），限界上下文就是边界
3. "聚合根怎么设计？" → 引用其他聚合根用ID；通过领域事件通信；聚合尽量小

---

*DDD不是"银弹"，而是复杂业务领域的"共同语言"。简单CRUD，别硬上DDD。*
