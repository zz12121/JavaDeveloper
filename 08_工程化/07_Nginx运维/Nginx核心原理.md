...
**Nginx 负载均衡 vs 客户端负载均衡（Ribbon/Dubbo）**：
...
Nginx 本身的功能相对固定，但 **OpenResty**（Nginx + LuaJIT）让你可以用 Lua 脚本扩展 Nginx，实现动态逻辑。
...
**Q：Nginx 为什么能处理高并发？**
"Nginx 采用事件驱动架构，每个 Worker 进程是单线程事件循环，用 epoll/kqueue 多路复用监听所有连接。有事件就绪时才处理，等待时不占 CPU。一个 Worker 可以处理数万个并发连接，内存占用极低。相比之下 Apache 的多进程模型，一个连接一个进程，并发高时线程切换和内存开销巨大。"

**Q：location 匹配规则？**
"优先级从高到低：精确匹配 `= ` 最高，然后是 `^~` 前缀匹配，再是 `~` 正则匹配（按配置顺序，第一个匹配即停止），最后是普通前缀匹配，兜底是 `/`。面试常考：给一段 Nginx 配置，问某个 URL 匹配到哪个 location。"

**Q：Nginx 反向代理和负载均衡？**
"反向代理是替服务器接收请求，客户端以为 Nginx 就是服务器，用来隐藏后端、SSL 卸载、缓存。负载均衡是把请求分发到多个后端，内置轮询、加权、IP 哈希、最少连接数等策略。Nginx 是服务端负载均衡，和客户端负载均衡（Ribbon）的区别是位置不同：Nginx 在网关层，Ribbon 在应用内。"

**Q：Nginx 怎么实现动静分离？**
"静态资源（HTML/CSS/JS/图片）直接由 Nginx 返回，不经过后端，极大减轻后端压力。`location` 匹配静态资源路径，用 `root` 或 `alias` 指定本地目录，再加 `expires` 设置浏览器缓存、`access_log off` 关闭访问日志。"

**Q：Nginx reload 的原理？**
"Master 进程收到 reload 信号后，重新加载配置文件，启动新的 Worker 进程，然后向旧 Worker 发送优雅关闭信号。旧 Worker 处理完当前请求后退出，新请求全部由新 Worker 处理。整个过程不中断连接，实现零停机部署。"

---

## 本文总结

**Nginx** 是高性能 Web 服务器和反向代理，核心要点：

- **架构**：Master-Worker 模型，事件驱动 + epoll 多路复用，一个 Worker 处理数万并发
- **反向代理**：`proxy_pass` 转发请求，一定要传 `X-Real-IP` 和 `X-Forwarded-For`
- **负载均衡**：轮询/加权/IP哈希/最少连接数，支持被动健康检查
- **Location 匹配**：`=` > `^~` > `~` > 前缀 > `/`，面试高频
- **HTTPS**：SSL 卸载在 Nginx 层完成，后端只用 HTTP
- **限流**：`limit_req_zone` + `limit_req`，防恶意请求
- **动静分离**：静态资源 Nginx 直返，不经过后端
- **OpenResty**：Nginx + Lua，实现动态逻辑（限流、认证、灰度）

**高频面试考点**：事件驱动架构原理、location 匹配规则、反向代理配置、动静分离。

---

**关联文档**：
- [[API网关设计]]（网关选型：Nginx vs Spring Cloud Gateway vs Kong）
- [[负载均衡算法]]（Nginx 负载均衡策略详解）
- [[HTTP协议详解]]（HTTPS/TLS 配置）
- [[高可用设计]]（Nginx 高可用： keepalived + VIP）
