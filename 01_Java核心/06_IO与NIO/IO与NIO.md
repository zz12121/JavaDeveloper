...
| 文档 | 覆盖内容 | 重点 |
|------|---------|------|
| [[01_Java核心/06_IO与NIO/BIO|BIO]] | 阻塞 IO 模型、ServerSocket/Socket、线程模型 | 一连接一线程的瓶颈、BIO 适用场景 |
| [[01_Java核心/06_IO与NIO/NIO|NIO]] | Channel/Buffer/Selector、epoll | Buffer 状态机（flip/clear/compact）、Selector 事件模型、空轮询 Bug |
| [[01_Java核心/06_IO与NIO/AIO|AIO]] | AsynchronousChannelGroup、CompletionHandler | 回调模型、与 NIO 的适用场景对比 |
| [[01_Java核心/06_IO与NIO/零拷贝|零拷贝]] | 传统 IO → mmap → sendfile 演进 | 4次拷贝→3次→2次，Kafka/RocketMQ 的零拷贝实践 |
| [[01_Java核心/06_IO与NIO/序列化|序列化]] | Serializable/Externalizable、transient、JSON 序列化 | writeObject/readObject 源码、serialVersionUID 兼容性、安全漏洞 |
