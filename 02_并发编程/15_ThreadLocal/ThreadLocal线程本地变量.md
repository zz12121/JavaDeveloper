...
`ThreadLocal` 的本质是**以空间换时间**——每个线程存一份副本，避免共享导致的锁竞争。

**核心要点**：
1. 数据存在 `Thread.threadLocals` 里，`ThreadLocal` 只是 key
2. `ThreadLocalMap` 用线性探测，不是 HashMap 的链表法
3. Entry 的 key 是弱引用（防止 `ThreadLocal` 对象泄漏），但 value 是强引用（需要手动 `remove()`）
4. **线程池场景必须 `remove()`**，否则内存泄漏 + 脏数据

**面试追问高发区**：
1. `ThreadLocal` 内存泄漏的完整机制（弱引用为什么还不够？）
2. 线程池里 `ThreadLocal` 的问题（为什么必须 `remove()`？）
3. `InheritableThreadLocal` vs `TransmittableThreadLocal`
4. `ThreadLocalMap` 为什么用线性探测而不是链表
