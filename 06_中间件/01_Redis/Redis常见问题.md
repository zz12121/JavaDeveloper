...
Redis的"常见问题"，本质都是在**性能、内存、一致性**之间做权衡：

1. **热Key** → 本地缓存副本 or Key拆分（牺牲一致性，换性能）
2. **大Key** → 拆分 or 异步删除（避免阻塞）
3. **过期策略** → 惰性+定期（CPU和内存的权衡）
4. **内存碎片** → 定期清理（Redis 4.0+支持在线清理）
5. **集群抖动** → 客户端重试（接受短暂不可用）

**面试高频追问**：
1. 热Key怎么解决？（本地缓存+Key拆分）
2. 大Key怎么拆分？（按hash取模拆成多个小Key）
3. Redis的过期策略是什么？（惰性删除+定期删除）
4. Redis内存碎片怎么清理？（Redis 4.0+的`memory purge`或配置主动碎片清理）
...
"热Key是指单个Key的访问量极高（比如每秒10万次），导致单个Redis实例扛不住。

**解决方案有3种**：

**第一种：本地缓存副本**
- 在应用本地用Caffeine缓存热Key的副本（TTL设短一点，比如10秒）
- 读的时候先读本地缓存，未命中再读Redis
- 优点：效果最好（完全挡住热Key请求）
- 缺点：数据可能不一致（本地缓存和Redis有延迟）

**第二种：Key拆分**
- 把热Key拆成10个副本（hot:key:1, hot:key:2, ...）
- 读的时候随机选一个
- 优点：简单，不需要本地缓存
- 缺点：写的时候要更新10个副本（可以用消息队列异步更新）

**第三种：Redis Cluster读取从节点**
- 如果是Cluster架构，读请求可以打到从节点（减轻主节点压力）
- 缺点：主从延迟可能导致读到旧值

我们生产环境用的是**第一种+第二种结合**：本地缓存挡大部分请求，Key拆分做兜底。"

**Q：Redis的大Key有什么问题？怎么解决？**

"大Key是指单个Key占用的内存很大（比如一个Hash有1000万个field）。

**问题**：
1. 删除时阻塞（O(n)操作，可能几百毫秒）
2. 网络传输慢（一次读可能传输几MB）
3. Redis Cluster不支持迁移大Key（限制512MB）

**解决方案**：

**第一种：拆分大Key**
- 按hash取模，拆成多个小Key
- 比如`user:1000:friends`（1000万个friendId）
- 拆成`user:1000:friends:part0`, `part1`, ... `part9`（每批100万元素）

**第二种：异步删除（Redis 4.0+）**
- 配置`lazyfree-lazy-server-del yes`
- 删除大Key时，Redis放到后台线程删除，不阻塞主线程

我们生产环境的实践：
- 避免大Key（设计阶段就拆分）
- 如果已经有大Key，用Redis 4.0+的异步删除功能
- 定期用`redis-cli --bigkeys`扫描大Key"

**Q：Redis的过期策略是什么？**

"Redis的过期策略是**惰性删除+定期删除**结合：

**惰性删除**：
- 访问某个Key时，检查是否过期
- 过期了就删除，返回null
- 优点：CPU友好（不主动扫描）
- 缺点：如果Key一直不被访问，会内存泄漏

**定期删除**：
- 定时任务（每秒10次，即每100ms一次）
- 随机选20个设置了过期时间的Key
- 删除已过期的Key
- 如果超过25%的Key已过期，再随机选20个（循环）
- 优点：防止惰性删除的"内存泄漏"
- 缺点：定时任务有开销

**为什么不用定时删除（到过期时间立即删除）？**
→ CPU开销太大（如果大量Key同时过期，CPU会爆）

**为什么不用定期扫描全部Key？**
→ 太慢（O(n)，阻塞主线程）

所以Redis用"惰性+定期"组合，在CPU和内存之间找平衡。"
...
| 常见问题 | 原因 | 解决方案 |
|---------|------|---------|
| 热Key | 单个Key访问量极高 | 本地缓存副本 / Key拆分 / 读从节点 |
| 大Key | 单个Key太大（>10KB） | 拆分 / Redis 4.0+异步删除 |
| 过期策略 | 惰性删除可能内存泄漏 | 惰性删除 + 定期删除组合 |
| 内存碎片 | 频繁更新/删除导致碎片 | Redis 4.0+的`memory purge` |
| 集群抖动 | 故障转移期间短暂不可用 | 客户端重试机制 |
| 网络带宽 | 吞吐量高，带宽打满 | 压缩数据 / Pipeline批量操作 |

**核心要点**：
1. 热Key用本地缓存+Key拆分解决
2. 大Key要拆分，或用Redis 4.0+的异步删除
3. 过期策略是"惰性+定期"组合
4. Pipeline减少网络RTT，但要控制批次大小

**面试关键**：
- 不只是知道"有什么问题"，要能说清楚**解决方案的细节**
- 热Key和大Key是生产环境最高频的问题，必须掌握
