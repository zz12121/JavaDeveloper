...
### 核心机制：ThreadPoolExecutor 的 7 个参数
...
public void execute(Runnable command) {
    ...
    int c = ct1.get();   // AtomicInteger，高 3 位 = 线程池状态，低 29 位 = 线程数
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            if (wc >= CAPACITY || wc >= (core ? corePoolSize : maximumPoolSize))
                return false;   // 超出限制，不创建
            if (compareAndIncrementWorkerCount(c))
                break retry;     // CAS 增加线程数成功，跳出外层循环
            c = ct1.get();     // CAS 失败，重试
        }
    }
...
### ThreadPoolExecutor vs Executors 工厂方法
...
| `newCachedThreadPool()` | `new ThreadPoolExecutor(0, Integer.MAX_VALUE, 60s, SynchronousQueue())` | 最大线程数无界，可能创建大量线程导致 OOM |
...
RejectedExecutionHandler handler = (task, executor) -> {
    ...
    // 方案3：阻塞提交（用 Runnable 的 get() 阻塞当前线程）
};
```
...
|--|------------|---------------|
| 接受新任务 | ❌ | ❌ |
| 执行队列任务 | ✅ 继续执行 | ❌ 不执行（返回未执行任务） |
| 中断运行中线程 | ❌ | ✅ 调用 `Thread.interrupt()` |
...
**参数配置的核心矛盾**：
- `corePoolSize` 小 → 任务多时频繁创建临时线程（开销大）
- `corePoolSize` 大 → 低负载时线程闲置（资源浪费）
- 队列大 → 抗压能力强，但延迟高，可能 OOM
- 队列小 → 延迟低，但容易触发拒绝

**面试追问高发区**：
1. `execute()` 的完整流程（核心 → 队列 → 临时 → 拒绝）
2. `corePoolSize` 和 `maximumPoolSize` 的区别（什么时候创建临时线程？）
3. 为什么 `submit()` 的异常会被吞掉（怎么正确处理？）
4. `shutdown()` vs `shutdownNow()`
5. 自定义 `ThreadFactory` 和拒绝策略
