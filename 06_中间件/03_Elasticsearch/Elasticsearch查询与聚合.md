...
**Q：Bool 查询里 must 和 filter 的区别？**
"must 会计算评分，结果不缓存；filter 不计算评分，结果会被 BitSet 缓存，性能更好。精确过滤条件（比如 status=1、price 范围）应该用 filter，全文搜索条件（比如 match content）用 must。"

**Q：ES 聚合和 MySQL GROUP BY 比有什么优势？**
"ES 聚合是分布式的，每个 Shard 并行计算，协调节点汇总，海量数据下性能远超 MySQL。而且 ES 支持嵌套聚合（在一个 Bucket 里再做聚合）和管道聚合（对聚合结果再做计算），表达方式比 SQL 更灵活。"

**Q：深度分页怎么解决？**
"from + size 方式深度分页性能极差，因为每个 Shard 要准备 from+size 条数据。解决：用 search_after（基于排序值翻页，推荐）或 scroll API（适合全量导出，不适合实时翻页）。"

**Q：为什么 text 字段不能做 terms 聚合？**
"text 字段被分词了，聚合会在分词后的词项上分组，结果没有业务意义。要对字段做聚合，必须用 keyword 类型（或 .keyword 子字段），它保留原始值，适合分组和精确匹配。"

## 本文总结

ES 查询分两大类：**叶子查询**（`match`、`term`、`range`）和 **复合查询**（`bool` 最常用）。

`bool` 查询的四个子句：`must`（匹配+评分）、`should`（匹配+加分）、`must_not`（不匹配）、`filter`（匹配+缓存，不评分）。**精确过滤优先用 filter**。

聚合分四大类：**Metric**（指标计算：sum/avg 等）、**Bucket**（分组：terms/range/date_histogram）、**Pipeline**（对聚合结果再计算）。

**最重要的最佳实践**：
- text 字段用 `match` 查询，keyword 字段用 `term` 查询/聚合
- 深度分页用 `search_after`，不要用 `from + size`
- 不需要原始文档时设 `"size": 0`，省掉 Fetch 阶段开销
