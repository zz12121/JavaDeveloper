...
"MVCC（多版本并发控制）是InnoDB实现非阻塞读的关键机制，核心是三个组件：

**第一，隐藏列**。InnoDB的每行数据都有两个隐藏列：
- `trx_id`：最近修改这行数据的事务ID
- `roll_pointer`：回滚指针，指向Undo Log中这个行的旧版本

**第二，Undo Log版本链**。每次修改一行数据，都会往Undo Log里写入旧值，并通过`roll_pointer`形成链式结构。不同事务可以根据需要读取不同版本的数据。

**第三，Read View（快照）**。事务执行快照读（普通SELECT）时，会创建一个Read View，记录当前活跃的事务ID列表。读数据时，根据Read View判断哪个版本可见：
- 如果数据版本的`trx_id`小于Read View的最小活跃事务ID，说明这个版本是在事务开始前就已提交的，可见
- 如果`trx_id`在活跃事务列表中，说明是未提交的事务修改的，不可见，需要沿着版本链往前找
- 如果`trx_id`大于Read View的最大事务ID，说明是事务开始后新提交的，不可见，需要往前找

通过这套机制，读操作不需要加锁，读写互不阻塞，大大提升了并发性能。"

**Q：Read View在RC和RR隔离级别下有什么区别？**

"Read View的创建时机不同，这直接决定了隔离级别的行为：

**READ COMMITTED级别**：每次执行快照读（普通SELECT）时，都会创建一个新的Read View。所以每次读都能看到别的事务已提交的最新数据，但可能产生不可重复读（同一个事务内，两次读结果不一样）。

**REPEATABLE READ级别**：事务开始时创建一个Read View，整个事务期间都复用这个Read View。所以整个事务内读到的数据版本是固定的，保证了可重复读。

这也是为什么RR能避免脏读和不可重复读，而RC只能避免脏读。"
...
**快照读（Snapshot Read）**：
- 普通SELECT语句（不带FOR UPDATE/LOCK IN SHARE MODE）
- 通过MVCC读历史版本，不需要加锁
- 读写互不阻塞，并发性能高
- RR级别下，整个事务读到的数据一致；RC级别下，每次读都是最新已提交版本

**当前读（Current Read）**：
- SELECT...FOR UPDATE、SELECT...LOCK IN SHARE MODE、INSERT、UPDATE、DELETE
- 读最新的已提交数据，并对读到的行加锁
- 用于保证数据的一致性（比如你读了某行要基于它做更新，必须读最新值并加锁防止别的事务修改）

一个常见的混淆是：'SELECT...LOCK IN SHARE MODE是不是快照读？' 不是，它虽然加的是共享锁，但读的是最新数据，属于当前读。"
...
| 概念 | 核心要点 |
|------|---------|
| 隐藏列 | `trx_id`（最近修改事务ID）+ `roll_pointer`（指向旧版本） |
| Undo Log版本链 | 每次修改写入旧值，形成链式结构 |
| Read View | 判断数据版本可见性的快照 |
| 快照读 | 普通SELECT，通过MVCC读历史版本，不加锁 |
| 当前读 | SELECT...FOR UPDATE等，读最新数据并加锁 |
| RC vs RR | RC每次读新建Read View，RR事务内复用同一个Read View |

**MVCC的本质**：通过保存数据的多个历史版本，让读操作能看到事务开始时的快照，从而实现非阻塞读，提升并发性能。

**局限性**：MVCC只能解决读写冲突（读写不阻塞），不能解决写写冲突（写写冲突需要靠锁机制解决）。
