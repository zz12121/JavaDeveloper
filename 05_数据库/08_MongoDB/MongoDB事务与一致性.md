...
**Q：MongoDB 支持事务吗？怎么用？**
"4.0 开始支持多文档事务，4.2 支持分片集群上的事务。用法和关系型数据库类似：用 `session.startTransaction()` 开启事务，操作加 `{ session }` 参数，成功后 `commitTransaction()`，失败后 `abortTransaction()`。但要注意性能代价，MongoDB 的最佳实践是用嵌入模型避免多文档事务，单文档操作是天然原子的。"

**Q：MongoDB 事务和 MySQL 事务有什么区别？**
"MySQL 是天生支持 ACID，事务是其核心能力；MongoDB 4.0 才支持多文档事务，且建议优先用嵌入模型避免事务。隔离级别上，MongoDB 的 `snapshot` 类似 MySQL 的 RR（可重复读），但没有 Serializable 级别。性能上，MySQL 的事务优化更成熟，高并发场景 MongoDB 的事务性能较差，要谨慎使用。"

**Q：Write Concern 和 Read Concern 是什么？**
"Write Concern 控制写操作需要多少节点确认：`w:1` 只写主节点就返回（快但有丢数据风险），`w:majority` 写大多数节点才返回（推荐）。Read Concern 控制读操作能看到什么样的数据：`local` 读最新（可能读未提交），`majority` 读大多数节点确认的数据（推荐），`snapshot` 快照读（事务内用）。两者组合使用，保证数据一致性。"

**Q：Change Stream 是什么？有什么用？**
"Change Stream 是 MongoDB 3.6+ 的功能，类似 MySQL 的 Binlog，可以监听集合的数据变更（insert/update/delete）。典型用途：数据同步（MongoDB → Elasticsearch）、实时通知（订单状态变更推送用户）、缓存失效（数据变更删除 Redis 缓存）。相比轮询，Change Stream 是事件驱动的，实时性好且开销小。"
...
- **单文档原子性**：天生支持，不需要事务（和 MySQL 最大的使用差异）
- **多文档事务**：4.0+ 支持，但**有性能代价**，优先用嵌入模型避免
- **Write Concern**：`w:1`（快）/ `w:majority`（推荐）/ `w:N`（最安全）
- **Read Concern**：`local` / `majority`（推荐）/ `snapshot`（事务内）
- **事务隔离级别**：`snapshot`（类似 RR，可重复读）
- **Change Stream**：监听数据变更，用于数据同步、实时通知、缓存失效
- **最佳实践**：事务尽量短小、避免跨分片事务、用重试逻辑处理临时失败

**高频面试考点**：MongoDB 事务 vs MySQL 事务对比、Write Concern/Read Concern、Change Stream 用途、嵌入模型 vs 多文档事务的选择。

---

**关联文档**：
- [[MongoDB核心概念与架构]]（副本集、分片、WiredTiger 存储引擎）
- [[ACID实现原理]]（MySQL 的 ACID 实现，对比 MongoDB）
- [[MVCC与事务处理]]（PostgreSQL 的 MVCC，对比 MongoDB 事务）
- [[MongoDB索引与查询优化]]（事务对索引的影响）
