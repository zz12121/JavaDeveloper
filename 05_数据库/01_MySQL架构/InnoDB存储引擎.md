...
InnoDB的设计精髓在于 **用Redo Log换性能，用Undo Log换灵活性**：

1. **WAL机制**（先写日志，再刷磁盘）：
   - 把随机写变成顺序写 → 性能提升10倍以上
   - 这是所有现代数据库的标配（PostgreSQL的WAL、Oracle的Redo Log）

2. **Buffer Pool**：
   - 用内存缓存热点数据 → 减少磁盘IO
   - 改进LRU算法（Young/Old区）→ 防止全表扫描污染缓存

3. **Undo Log + MVCC**：
   - 读写不阻塞 → 大幅提升并发性能
   - 这是InnoDB在OLTP场景下表现优异的关键
...
**面试回答思路**：
- 先说InnoDB的核心组件（Buffer Pool、Redo Log、Undo Log、行锁）
- 然后解释为什么需要这些组件（磁盘IO慢、事务ACID要求、并发控制）
- 如果面试官追问，再深入讲WAL机制、MVCC版本链、Next-Key Lock

**类比**：
- Buffer Pool = 工作台（把常用工具放在手边，不用每次去仓库拿）
- Redo Log = 记账本（先记下来，有空再整理到总账）
- Undo Log = 草稿纸（写错了可以擦掉重来）
- 行锁 = 给文件加锁（只锁自己正在改的那一页，别人可以改其他页）

---

*InnoDB是MySQL最核心的组件。不理解InnoDB，就无法理解"事务是怎么实现的"、"为什么索引能加速查询"、"死锁是怎么发生的"。*
