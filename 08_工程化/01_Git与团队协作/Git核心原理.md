...
Git 最深刻的设计是**内容寻址 + 不可变对象**：

- 所有对象一旦创建，内容就不会改变（SHA-1 决定了内容 → 哈希的绑定）
- 所有的「修改」都是创建新对象，旧对象还在
- 这就让 Git 的「撤销」「恢复」「reflog」成为可能——因为旧数据永远不会丢失（直到 GC）

这也是为什么 Git 仓库会随着时间膨胀——它不是「存储差异」，而是「存储所有快照」。Git 的 GC（`git gc`）会定期打包相似对象，但最终仓库大小还是会比 SVN 大。

**面试高频考点**：
1. Git 对象模型（blob/tree/commit 的关系）—— 必须能画出来
2. `git reset` 三个模式的区别 —— 高频
3. 分离头指针是什么，如何恢复 —— 中等频率
4. `git rebase` vs `git merge` 的区别和适用场景 —— 高频
5. 为什么 Git 分支这么轻量 —— 理解引用（ref）的本质

*Git 是目前软件工程中最成功的分布式系统案例之一。理解它，不只是为了用命令，而是理解「分布式协作」的设计哲学。*
