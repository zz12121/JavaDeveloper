...
Dockerfile 最佳实践的本质是**控制复杂度**：

- **镜像大小** → 影响传输速度、存储成本、启动速度
- **构建速度** → 影响开发体验、CI/CD 吞吐量
- **安全性** → 影响是否被入侵、被入侵后损失多大

**我总结的 Dockerfile 优化三步法**：

1. **先跑通**：写出能用的 Dockerfile
2. **再优化**：用多阶段构建减小镜像，用缓存加速构建
3. **最后加固**：不用 root、设只读文件系统、加健康检查

**面试高频考点**：

1. 多阶段构建是什么，为什么要用？—— 必问
2. 怎么减小 Docker 镜像大小？—— 多阶段、合并 RUN、.dockerignore
3. Dockerfile 中 `CMD` 和 `ENTRYPOINT` 的区别？—— 高频
4. 怎么加速 Docker 镜像构建？—— 利用缓存、.dockerignore、多阶段
5. 你们项目的 Dockerfile 怎么写的？—— 开放题，结合实战

*Dockerfile 是「基础设施即代码」（Infrastructure as Code）的一部分。写得好，部署效率高 10 倍；写得差，每次部署都是一场灾难。*
