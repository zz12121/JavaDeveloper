...
**Q1：PostgreSQL 的进程模型和 MySQL 的线程模型，各有什么优缺点？**

> PostgreSQL 用进程模型，每个连接一个进程，稳定性高（一个连接 Crash 不影响其他连接），但并发能力受限（进程比线程重，通常最多几百个连接）。MySQL 用线程模型，轻量，支持高并发（上万连接）。生产环境 PostgreSQL 必须用连接池（如 PgBouncer）来复用连接。

**Q2：PostgreSQL 的堆表和 MySQL 的索引组织表，有什么区别？**

> 堆表的索引指向行的物理位置（ctid），所有索引都是二级索引，都需要"回堆"取数据。索引组织表的主键索引就是数据，二级索引存主键值，也需要回表。堆表的优势是：多索引维护成本低（行位置不变，索引不用更新），适合有很多索引的场景。

**Q3：为什么 PostgreSQL 需要 VACUUM，MySQL 不需要？**

> PostgreSQL 的 MVCC 实现是把旧版本行留在堆表里（通过 xmin/xmax 标记），不自动清理。MySQL 的 MVCC 是把旧版本存在 Undo Log 里，Undo Log 可以被覆盖或后台清理。PostgreSQL 需要 VACUUM 来清理死元组，否则会导致表膨胀和 XID wraparound。

**Q4：WAL 和 Redo Log 有什么区别？**

> WAL 是物理+逻辑混合日志（记录"对哪个块的哪个元组做什么操作"），Redo Log 是纯物理日志（记录"对哪个数据页的哪个偏移量写什么数据"）。WAL 的物理复制比 MySQL 的 binlog 逻辑复制更高效（并行回放更友好）。

**Q5：PostgreSQL 的主从复制是怎么工作的？**

> 主库写 WAL，WAL 通过流复制（Streaming Replication）发给从库，从库的 Startup 进程重放 WAL（类似 MySQL 的 SQL 线程，但支持并行回放）。默认是异步复制（可能丢最后一个事务），可以配置同步复制（synchronous_commit = on）。
...
| 核心概念 | 要点 |
|----------|------|
| **进程模型** | 一连接一进程，稳定但并发有限，必须用连接池（PgBouncer） |
| **堆表存储** | 索引指向 ctid，所有索引都是二级索引，TOAST 机制处理大字段 |
| **WAL** | 预写日志，保证持久性，物理+逻辑混合，支持流复制 |
| **Checkpoint** | 脏页刷盘，调优 checkpoint_completion_target=0.9 避免 IO 尖峰 |
| **VACUUM** | 清理死元组，防止表膨胀和 XID wraparound，必须开启 AutoVacuum |
| **内存配置** | shared_buffers=25%RAM，work_mem 小心设置（并发会翻倍） |

**PostgreSQL 的定位**：适合复杂查询、数据分析、需要 GIS/JSONB/全文搜索的场景。高并发简单读写场景，MySQL 更合适。
