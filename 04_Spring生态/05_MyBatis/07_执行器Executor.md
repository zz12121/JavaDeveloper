...
Executor 是 MyBatis 的「执行引擎」，有三种实现：
- `SimpleExecutor`：每次新建 Statement（默认，最简单）
- `ReuseExecutor`：同 SQL 复用 Statement（减少创建开销）
- `BatchExecutor`：攒批执行（批量写入性能最好）

**实际项目建议**：
- 普通场景用默认（Simple）✅
- 批量插入用 BatchExecutor ✅（但记得 flush）
- ReuseExecutor 用得少（效果有限，且可能 Statement 泄漏）⚠️

---

## 下一步

打开 [[Executor]]，补充 `[[双向链接]]`：「Executor 是 MyBatis 的执行层，Simple/Reuse/Batch 三种实现，批量写入必用 BatchExecutor」。
