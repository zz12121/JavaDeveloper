...
API网关的核心价值是**统一处理横切逻辑**（认证、日志、限流），让微服务只关心业务逻辑。

但实际生产中，网关的选型要权衡：
- **Spring Cloud全家桶** → Spring Cloud Gateway（集成方便）
- **高性能要求** → Nginx 或 Kong
- **云原生环境** → 用K8s Ingress（基于Nginx或Envoy）
...
**面试最高频追问**：
1. "API网关的核心能力？" → 统一入口、认证、路由、限流、熔断
2. "Spring Cloud Gateway vs Nginx？" → 功能完备性 vs 性能
3. "网关怎么实现灰度发布？" → 权重路由（Weight谓词）

---

*API网关是微服务的"门卫"——所有请求都从它这里进，它决定谁能进、去哪、快慢。*
