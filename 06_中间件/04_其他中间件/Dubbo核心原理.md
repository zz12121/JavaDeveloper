...
**Q：Dubbo 的架构组成？**
"Dubbo 有四个核心角色：Provider（服务提供方）、Consumer（服务消费方）、Registry（注册中心）、Monitor（监控中心）。Consumer 从 Registry 订阅服务地址，拿到地址列表后**直接调用 Provider，不经过注册中心**，这是 Dubbo 的高性能设计。"

**Q：Dubbo 的负载均衡策略？**
"默认是加权随机。还有加权轮询、一致性 Hash（适合有状态缓存）、最少活跃调用数（适合处理时间差异大的场景）。通过 SPI 机制实现的，可以自定义扩展。"

**Q：Dubbo 和 Spring Cloud 的区别？**
"核心区别是定位不同。Dubbo 是 RPC 框架，专注服务调用，性能高但需要自己组装其他组件。Spring Cloud 是微服务全家桶，基于 HTTP 协议，功能全但性能不如 RPC。国内用 Dubbo 较多，国际用 Spring Cloud 较多。现在很多项目是 Dubbo + Spring Cloud 混用。"

**Q：Dubbo 的 SPI 机制？**
"SPI 是 Dubbo 的可插拔扩展机制。和 Java SPI 不同，Dubbo SPI 支持按需加载（不全量加载）、依赖注入、缓存。Dubbo 的负载均衡、序列化、协议、线程池等都通过 SPI 实现，用户可以自定义替换而不改源码。这是 Dubbo 架构的核心设计。"

**Q：Dubbo 调用失败怎么办？**
"Dubbo 提供了 6 种集群容错策略。默认 Failover，失败自动重试其他 Provider，适合读操作。写操作必须用 Failfast，快速失败不重试，否则会重复执行。还有 Failsafe（忽略异常）、Failback（定时重试）、Forking（并行调用）、Broadcast（广播调用）。"

**Q：为什么 Dubbo 用 Netty？**
"Netty 封装了 Java NIO 的复杂性（半包/粘包/空闲检测/优雅关闭等），提供了主从 Reactor 线程模型和高性能的 ByteBuf。Dubbo 不用自己造网络通信的轮子，专注在 RPC 语义上。"

---

## 本文总结

**Dubbo** 是阿里开源的高性能 Java RPC 框架，核心要点：

- **架构**：Provider/Consumer/Registry/Monitor 四角色，Consumer 直连 Provider（不经注册中心）
- **SPI**：可插拔扩展机制，负载均衡/序列化/协议/线程池都可替换，是 Dubbo 的灵魂
- **负载均衡**：加权随机（默认）/ 轮询 / 一致性Hash / 最少活跃数
- **容错策略**：Failover（读）/ Failfast（写）/ Failsafe / Failback / Forking / Broadcast
- **协议**：默认 dubbo 协议（TCP + NIO + Hessian2），也可切换 REST/gRPC
- **线程模型**：IO 线程（Netty）+ 业务线程池（默认 200），IO 线程不做业务
- **对比**：Dubbo 是高性能 RPC 框架，Spring Cloud 是微服务全家桶，按场景选择

**高频面试考点**：SPI 机制、负载均衡策略、集群容错策略、与 Spring Cloud 对比。
