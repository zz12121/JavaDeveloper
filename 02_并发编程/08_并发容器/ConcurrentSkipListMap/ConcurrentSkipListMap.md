...
```
Q1: 跳表是什么？为什么用跳表而不是红黑树？
→ 跳表 = 多层有序链表 + 随机层数
→ 查找 O(log n)，和红黑树一样
→ 但实现更简单，并发更友好（局部修改 vs 大范围旋转）
→ 天然支持范围查询（链表顺序遍历）

Q2: ConcurrentSkipListMap 的并发实现原理？
→ CAS 无锁：插入时 CAS 更新前驱节点的 next 指针
→ 逻辑删除 + 物理删除两阶段
→ 不用分段锁，不用红黑树重平衡

Q3: 什么场景下用 ConcurrentSkipListMap？
→ 排行榜（按分数排序 + 范围查询）
→ 时间线（按时间戳排序 + 时间范围查询）
→ 滑动窗口统计

Q4: 和 ConcurrentHashMap 的区别？
→ CHM 无序，SkipListMap 有序
→ CHM 查找 O(1)，SkipListMap O(log n)
→ CHM 不支持范围查询，SkipListMap 支持
→ 大部分场景用 CHM（性能更好），需要排序时用 SkipListMap
```

### 一句话总结

```
ConcurrentSkipListMap 是"并发 + 有序"的唯一选择。
跳表用概率平衡代替严格的旋转平衡，换取了更简单的实现和更好的并发性能。
当你需要按键排序的并发 Map 时，它就是答案——不需要犹豫。
```
