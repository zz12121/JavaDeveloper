...
### Q1: 什么是 IO 多路复用？为什么比 BIO 高效？

**答**：IO 多路复用通过一个线程同时监听多个 fd（文件描述符），当某个 fd 就绪（可读/可写）时才通知应用程序处理。相比 BIO（一连接一线程），IO 多路复用只需要少量线程就能处理大量连接，避免了线程上下文切换的开销。底层通过 epoll 的红黑树+双向链表实现 O(1) 的事件通知。

### Q2: select、poll、epoll 的区别？

**答**：
- **select**：数组实现，最大 1024 个 fd，每次全量拷贝到内核，O(n) 遍历
- **poll**：链表实现，无数量限制，但仍然是全量拷贝 + O(n) 遍历
- **epoll**：红黑树管理 fd，事件回调机制，只返回就绪 fd，O(1)

### Q3: 什么是零拷贝？有哪些实现方式？

**答**：零拷贝减少数据在内核空间和用户空间之间的拷贝次数。Java 中 `FileChannel.transferTo()` 底层调用 sendfile，数据不经过用户空间。mmap 则是将内核缓冲区映射到用户空间，减少一次 CPU 拷贝。Netty 的 FileRegion 也利用了零拷贝。

### Q4: epoll 的 ET 模式和 LT 模式有什么区别？

**答**：LT（水平触发）只要缓冲区有数据就通知，支持非阻塞读但可以不一次读完；ET（边缘触发）只在数据从无到有时通知一次，**必须循环读到 EAGAIN**。ET 更高效（减少系统调用），但编程更复杂。Redis 和 Nginx 使用 epoll 的 ET 模式。

### Q5: Java NIO 和 Netty 的关系？

**答**：Java NIO 提供了 Channel/Buffer/Selector 基础 API，但存在 Buffer 管理复杂、epoll 空轮询 Bug 等问题。Netty 在 NIO 之上封装了 EventLoop（Reactor）、ByteBuf（自动内存管理）、ChannelPipeline（责任链）等抽象，解决了原生 NIO 的各种坑，是生产环境 Java 网络编程的事实标准。

---

> **面试加分项**：能清晰描述五种 IO 模型的等待/拷贝阶段差异，能说出 epoll 的红黑树+链表原理，能对比零拷贝的几种技术，能解释为什么 Redis/Nginx/Netty 都选择了 epoll。
