...
**Q：EXPLAIN 的 type 字段有哪些值，按性能怎么排序？**

"从好到坏是：

`system`：表只有一行（系统表），这是 const 的特例。

`const`：主键或唯一索引的等值查询，最多返回一行，性能最好。

`eq_ref`：主键或唯一索引的 JOIN，对于驱动表的每一行，被驱动表只有一行匹配。

`ref`：非唯一索引的等值查询，可能返回多行。

`range`：范围查询，比如 BETWEEN、IN、>、< 等，用上了索引。

`index`：全索引扫描，比 ALL 好一点，因为只扫描索引树，不需要回表。

`ALL`：全表扫描，最差，需要优化。

面试时，目标是让 type 至少达到 `range`，最好是 `ref` 或 `eq_ref`。"

**Q：Extra 字段有哪些常见值，分别代表什么？**

"几个高频考点：

`Using index`：覆盖索引，查询的所有列都在索引里，不需要回表，性能很好。

`Using index condition`（ICP）：索引下推，在存储引擎层就把 WHERE 条件过滤了，减少回表次数。

`Using filesort`：需要额外的排序操作，没有利用索引的有序性。优化方向是建联合索引让 ORDER BY 走索引。

`Using temporary`：需要创建临时表，常见于 GROUP BY 无索引的场景。优化方向是给 GROUP BY 的字段加索引。

`Using where`：在 Server 层进行过滤，如果和 `type=ALL` 同时出现，说明全表扫描后过滤，性能很差。"

**Q：如何通过 EXPLAIN 判断索引是否有效？**

"看三个字段：

第一，`key` 字段。如果是 NULL，说明没用上任何索引，全表扫描。

第二，`rows` 字段。预估扫描的行数，如果和表的总行数接近，说明索引效果不好。

第三，`key_len` 字段。判断联合索引实际用了几列。比如联合索引 `(a, b, c)`，如果 `key_len` 只等于 a 的长度，说明只用了第一列。

另外，`Extra` 里的 `Using index` 是好事，说明覆盖索引生效，不需要回表。"
...
| 字段 | 核心作用 | 优化目标 |
|------|---------|----------|
| `type` | 访问类型，判断是全表扫还是索引查 | 至少达到 `range`，争取 `ref` |
| `key` | 实际使用的索引 | 不能为 NULL |
| `rows` | 预估扫描行数 | 越小越好 |
| `key_len` | 索引用到的字节数 | 判断联合索引的利用率 |
| `Extra` | 附加信息 | 争取 `Using index`，避免 `Using filesort` |

**核心要点**：
1. `type=ALL` 是首要优化目标，加索引解决
2. `Extra: Using filesort` 说明 ORDER BY 没走索引，建联合索引解决
3. `key_len` 可以判断联合索引的利用率
4. `EXPLAIN ANALYZE`（MySQL 8.0+）可以看到实际执行耗时
5. `rows` 是估算值，不准确时执行 `ANALYZE TABLE`

**工具链**：
```
慢查询日志 → mysqldumpslow/pt-query-digest 找出慢 SQL
    → EXPLAIN 分析执行计划
    → 加索引/改写 SQL
    → EXPLAIN 验证
    → 上线后持续监控
