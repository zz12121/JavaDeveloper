...
**Q：ES 集群怎么保证高可用？**
"三层：1) Master 选举（Raft 协议，超过半数选票才当选，防脑裂）；2) Shard 副本机制（Primary 挂了，Replica 提升为 Primary）；3) 数据写入可以用 wait_for_active_shards=all 保证所有副本确认。三层配合，保证集群高可用和数据不丢。"

**Q：ES 的 JVM 堆内存怎么设置？**
"两个原则：1) 不超过物理内存的 50%（留给 OS 做文件系统缓存，这是 ES 查询快的关键）；2) 不超过 32GB（压缩指针阈值，超过反而性能下降）。推荐：64GB 物理内存 → JVM 堆 32GB，剩余 32GB 给 OS 缓存。"

**Q：Shard 数怎么规划？**
"单个 Primary Shard 10GB~50GB 是最优范围。太小（比如 3GB 数据用 10 个 Shard）会导致每个 Shard 太小，Lucene 实例开销大，搜索时合并结果也慢；太大（比如 100GB/Shard）会导致单个 Shard 恢复和迁移慢。Shard 数一旦设定不能直接修改，要用 Rollover + Alias 管理数据增长。"

**Q：ES 和 Redis Cluster 的高可用方案有什么区别？**
"ES 用 Raft 协议选 Master，中心化协调，元数据管理强；Redis Cluster 用 Gossip 协议，去中心化，每个节点平等。ES 的 Shard 分配策略更复杂（考虑磁盘空间、节点标签等），Redis Cluster 的 Slot 分配相对简单。两者都是高可用，但架构思路不同。"

## 本文总结

ES 高可用三层保障：Master 选举（Raft，防脑裂）、Shard 副本（Primary 故障自动切换）、`wait_for_active_shards`（写入一致性保证）。

**性能调优关键**：
- JVM 堆 ≤ 32GB 且 ≤ 50% 物理内存（留给 OS 缓存）
- 单个 Shard 10GB~50GB（Shard 数不能直接修改，要提前规划好）
- 用 Bulk API 批量写入，减少网络 Round-Trip
- 时序数据用 ILM（Index Lifecycle Management）自动管理生命周期

生产环境 **Master 节点和数据节点分离**，避免 Master 被搜索请求拖垮。

**Rollover + Alias** 是解决"Shard 数不能修改"问题的标准方案，时序数据必备。
