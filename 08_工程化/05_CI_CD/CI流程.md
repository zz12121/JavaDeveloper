...
CI 最容易被低估的价值是**「强制质量门禁」**。

很多团队配了 CI，但只是「跑跑测试，失败了也不影响合并」——这等于没配。CI 必须和**分支保护规则（Branch Protection Rules）** 结合：

```
分支保护规则（GitHub/GitLab 都支持）：
  ✅ 禁止直接 push 到 main
  ✅ PR 必须至少 1 人 Review 并 Approve
  ✅ CI 必须通过（Status Check 通过）
  ✅ 合并前必须重新基于 main（避免合并后冲突）
```
...
1. **编译检查**（必须，5 分钟搞定）
2. **单元测试**（必须，覆盖核心逻辑）
3. **Lint / 代码规范**（高优先级，自动化「代码审美」）
4. **测试覆盖率门禁**（中优先级，防止覆盖率下降）
5. **集成测试**（中优先级，验证模块间调用）
6. **安全扫描**（低优先级，可以定期跑，不必每次提交都跑）

**面试高频考点**：

1. CI 的作用是什么？—— 快速反馈、强制规范、防止「我机器上能跑」
2. CI 和 CD 的区别？—— CI 是持续集成（构建+测试），CD 是持续部署（自动发布）
3. 你们项目的 CI 流水线是怎么配的？—— 开放题，结合实战回答
4. 怎么加速 CI 构建？—— 缓存、并行、增量构建
5. CI 失败了怎么办？—— 修复 → 推送到同一 PR → CI 自动重新运行

*CI 是「质量内建」的第一步。没有 CI 的团队，代码质量完全依赖开发者自觉——而人性是不可靠的。*
