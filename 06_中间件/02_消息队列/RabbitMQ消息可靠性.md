...
**Q：RabbitMQ 怎么保证消息不丢？**
"三层保障：生产端开启 Confirm 机制 + Return 机制，确保消息到达 Broker 且路由成功；Broker 端消息持久化（deliveryMode=2）+ 仲裁队列（多节点复制）；消费端关闭自动 Ack，处理完后手动 basicAck。三层都做到，可以保证消息不丢。"

**Q：Confirm 机制和 Transaction 有什么区别？**
"Transaction 是同步阻塞的，每条消息都要等 Broker 确认，性能极差，比非事务模式慢 250 倍。Confirm 是异步的，生产者继续发下一条，Broker 异步通知哪些收到了。生产环境只用 Confirm。"

**Q：手动 Ack 有什么要注意的？**
"两个坑：1) 一定要调用 basicAck，否则消息一直 Unacked，内存会爆；2) 处理失败时，basicNack 的 requeue 参数要小心，如果消费一直失败，requeue=true 会形成死循环，应该用死信队列。"

**Q：RabbitMQ 和 Kafka 在可靠性上怎么选？**
"RabbitMQ 的 Confirm 机制是逐条确认的，粒度更细，适合金融等对可靠性要求极高的场景，但吞吐量相对低（万级 TPS）。Kafka 用 acks=all + ISR 机制，吞吐量极高（百万级 TPS），可靠性也够用。简单说：要极致可靠用小批量，要极致吞吐用 Kafka。"

## 本文总结

RabbitMQ 消息可靠性 = **三层保障**：

1. **生产端**：Confirm 机制（异步确认到达）+ Return 机制（路由失败通知）
2. **Broker 端**：消息持久化（deliveryMode=2）+ 仲裁队列（多节点复制，Raft 协议）
3. **消费端**：手动 Ack（basicAck）+ 幂等消费 + 死信队列兜底

Transaction 机制性能极差（慢 250 倍），生产环境只用 Confirm。

`basicAck` 必须确保被调用（用 finally 块），否则 Unacked 消息堆积会导致内存泄漏。

`basicNack(requeue=true)` 要小心死循环，失败消息应该进死信队列。

仲裁队列（Quorum Queue）是 RabbitMQ 3.8+ 推荐的高可用方案，替代旧版镜像队列，用 Raft 协议保证数据一致性。
