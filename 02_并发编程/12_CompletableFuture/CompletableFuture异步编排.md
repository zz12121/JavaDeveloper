...
CompletableFuture 是现代 Java 异步编程的核心工具。它的价值不在于"异步执行任务"（线程池就能做），而在于**编排多个异步任务的依赖关系**。

**理解 CompletableFuture 的关键认知**：

1. **它本质是事件驱动的链式回调**——每个 then* 方法都注册一个回调，前置完成时自动触发
2. **thenCompose vs thenCombine** 是最高频的考点——顺序依赖 vs 并行合并
3. **必须指定线程池**——commonPool 是全局共享的，生产环境不能依赖
4. **异常处理是链式的**——exceptionally/handle 放在链的最后统一处理
5. **join() 比 get() 更常用**——不需要处理 checked exception

**面试中 CompletableFuture 的高频考点**：

1. **thenApply vs thenAccept vs thenRun**（转换/消费/后置）
2. **thenCompose vs thenCombine**（顺序依赖 vs 并行合并）
3. **allOf vs anyOf**（等待全部 vs 等待任意）
4. **exceptionally vs handle vs whenComplete**（异常处理的区别）
5. **默认线程池的问题**（为什么必须指定自定义线程池）
6. **thenApply vs thenApplyAsync**（同步 vs 异步回调）
7. **实际场景**（多接口聚合查询、超时降级）