...
> 详细实现见 [[../07_分布式与架构/02_分布式锁/ZooKeeper分布式锁]]（临时顺序节点 + Watch 前一个节点）
...
> ZAB 协议有两种模式。崩溃恢复模式：集群启动或 Leader 崩溃时，选举新 Leader，确保超过半数的节点数据一致。消息广播模式：Leader 接收写请求，生成 Proposal 广播给所有 Follower，超过半数 ACK 后才 Commit。这样保证所有节点看到相同的提交顺序，防止脑裂。

**Q2：Watch 机制为什么只触发一次？有什么优缺点？**

> Watch 是一次性的是 ZK 的设计选择。好处是服务端不需要维护大量订阅状态，减少复杂度，故障恢复快。坏处是客户端要自己管理重新注册，容易漏。etcd v3 改用 gRPC 流实现永久 Watch，是另一种权衡。

**Q3：为什么 ZooKeeper 是 CP 系统，不是 AP？**

> ZK 优先保证一致性和分区容错性，牺牲部分可用性。当网络分区时，少数派节点无法达成 Quorum，无法写入，对外表现为"不可用"。但保证了数据一致性。Redis 是 AP，主从切换时可能丢数据，但高可用性好。

**Q4：什么是羊群效应，怎么解决？**

> 羊群效应是指：一个节点变化，导致大量客户端同时被唤醒，网络风暴。比如 100 个客户端都 Watch 同一个配置节点，配置一变，100 个客户端同时来读。解决：用 Curator 的 Cache 封装（内部合并了重复的 Watch），或者控制 Watch 粒度（每个客户端 Watch 自己的节点）。

**Q5： sessionTimeout 怎么设置？太短或太长有什么问题？**

> 建议设为 2×tickTime（默认 4000ms）。太短：网络抖动就会导致会话失效，临时节点被误删。太长：客户端真 Crash 了，要等很久才能被检测到，故障转移慢。GC 停顿长的应用（用 CMS 垃圾回收器），要适当调大 sessionTimeout。

**Q6：ZooKeeper 和 etcd 的核心区别？**

> 一致性协议不同：ZK 用 ZAB，etcd 用 Raft（更易于理解）。Watch 机制不同：ZK 的 Watch 是一次性的，etcd v3 的 Watch 是永久的（基于 gRPC 流）。生态不同：ZK 是 Hadoop/Dubbo 生态，etcd 是 K8s 生态。新项目如果跑在 K8s 上，优先用 etcd。

**Q7：用 ZooKeeper 做配置中心有什么优缺点？**

> 优点：Watch 机制实现配置实时推送；强一致保证所有客户端看到相同配置；临时节点可做服务发现。缺点：Znode 存储上限 1MB，不适合大配置；Watch 一次性，客户端要自己管理重注册；没有原生的配置版本管理（要自己实现）。大公司通常用 Nacos/Apollo 做配置中心。

**Q8：ZooKeeper 还有什么典型应用场景？**

> 除了服务注册发现和配置中心，还有：分布式锁（临时顺序节点）、Leader 选举（Curator LeaderSelector）、分布式队列（持久顺序节点）、分布式 ID 生成（持久顺序节点 + 截取序号）、屏障（Barrier，所有节点就绪后才继续）。
...
| 核心概念 | 要点 |
|----------|------|
| **Znode** | 4 种类型，临时节点实现服务注册，顺序节点实现分布式锁 |
| **Session** | 会话超时导致临时节点删除，sessionTimeout 要合理配置 |
| **Watch** | 一次性监听，触发后需重新注册，避免服务端维持大量状态 |
| **ZAB 协议** | 崩溃恢复（选举）+ 消息广播（写请求），超半数确认防脑裂 |
| **Leader 选举** | ZXID 最大优先，ZXID 相同比 myid |
| **Curator** | 生产必用，封装了连接管理、重试、分布式锁、Leader 选举 |
| **集群规划** | 奇数节点（3/5/7），Observer 扩展读能力不改变 Quorum |

**ZooKeeper 的定位**：分布式协调基础设施（CP 系统），适合强一致场景（选主、配置管理、分布式锁）。高并发场景（如秒杀）用 Redis，服务发现新项目用 Nacos。
