...
**Q：RabbitMQ 的 Exchange 有哪几种类型？**
"四种：Direct（精确匹配 routingKey）、Topic（通配符匹配）、Fanout（广播到所有绑定队列）、Headers（根据消息头匹配，用得少）。80% 的场景用 Direct 或 Topic。"

**Q：RabbitMQ 怎么保证消息不丢？**
"三层：1) 生产端开启 Confirm 机制，确保消息到达 Broker；2) 消息和 Queue 都设为持久化（deliveryMode=2 + durable=true）；3) 消费端关闭自动 Ack，处理完后手动 basicAck。三层都做到，可以保证消息不丢。"

**Q：RabbitMQ 和 Kafka 怎么选？**
"RabbitMQ 适合业务系统的内部通信，低延迟、路由灵活，但吞吐量不大（万级 TPS）。Kafka 适合日志、流处理，吞吐量极高（百万级 TPS），但延迟相对高，路由能力弱。简单说：做业务用 RabbitMQ，做数据管道用 Kafka。"

**Q：什么是死信队列？**
"消息变成'死信'有三种情况：1) 消息被拒绝（basicReject/basicNack）且不重新入队；2) 消息 TTL 过期；3) 队列达到最大长度。死信会被转发到配置的 Dead Letter Exchange，进入死信队列，方便后续人工处理或延迟重试。这是处理失败消息的标准做法。"

## 本文总结

RabbitMQ 的核心架构：生产者 → Exchange（路由） → Queue → 消费者。Exchange 的四种类型：Direct（精确匹配）、Topic（通配符）、Fanout（广播）、Headers（消息头匹配，少用）。

高可靠三层保障：生产端 Confirm 机制、消息 + Queue 持久化、消费端手动 Ack。

RabbitMQ 是 Push 模型（低延迟），Kafka 是 Pull 模型（高吞吐）。两者不是替代关系，是互补关系。

**推荐配置**：生产端开启 Confirm、消息持久化、消费端手动 Ack、设置合理的 prefetch（限流）、失败消息进死信队列。

集群模式下，用**仲裁队列（Quorum Queue）**替代传统的镜像队列，获得更好的可用性和数据安全性。
