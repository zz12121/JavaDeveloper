...
分布式ID的核心需求是**全局唯一 + 趋势有序 + 高性能**。

- **UUID**：简单但不适合做主键（无序导致索引分裂）
- **数据库自增**：简单但性能瓶颈明显（每次访问DB）
- **号段模式**：平衡了性能和依赖（Leaf的默认方案）
- **雪花算法**：性能最高（无网络IO），但时钟回拨是致命问题

实际选型：
- **内网系统，对时钟敏感** → 号段模式（Leaf）
- **高并发，时钟可控** → 雪花算法
- **跨云部署，时钟不可控** → 类雪花（UidGenerator）

**面试最高频追问**：
1. "雪花算法为什么有序？" → 时间戳在高位，毫秒级递增保证趋势有序
2. "时钟回拨怎么解决？" → 抛异常、等时钟追上来、用ZK时间戳
3. "Leaf怎么优化号段模式？" → 双缓冲，10%时异步加载下一段

---

*分布式ID的选择，本质是在「性能」「有序性」「时钟依赖」之间做trade-off。*
