...
CD 最容易被忽视的是**「自动化测试覆盖率」**。没有完善的自动化测试，CD 就是「自动把 bug 发布到生产环境」。

**我的 CD 配置清单**：

```
✅ CI 通过才能触发 CD（单元测试、集成测试全覆盖）
✅ 镜像标签用 Git SHA（可追溯、可回滚）
✅ 滚动更新配置合理（maxUnavailable=0）
✅ 健康检查配置正确（readinessProbe + livenessProbe）
✅ 冒烟测试（Smoke Test）自动化
✅ 失败自动回滚 + 告警通知
✅ 敏感信息用 Secrets/Variables（不进 Git）
```

**面试高频考点**：

1. CD 和 CI 的区别 —— 高频
2. 零停机发布有哪些方式？—— 滚动更新、蓝绿、金丝雀
3. 你们项目的 CD 流水线是怎么配的？—— 开放题，结合实战
4. 怎么保证 CD 的安全性？—— 敏感信息管理、镜像扫描、审批流程
5. 滚动更新的原理是什么？—— 结合 `maxSurge` 和 `maxUnavailable` 讲

*CD 是「工程成熟度的试金石」。没有 CD 的团队，发布就是一场赌博。*
