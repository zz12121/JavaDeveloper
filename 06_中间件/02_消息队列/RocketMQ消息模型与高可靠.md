...
**Q：RocketMQ 的事务消息是怎么工作的？**
"分三步：1) 发半消息（对消费者不可见）；2) 执行本地事务；3) 根据本地事务结果 Commit 或 Rollback。如果 Commit/Rollback 失败了，Broker 会回查（checkLocalTransaction），根据本地事务状态决定提交还是回滚。这样保证了本地事务和消息发送的原子性。"

**Q：RocketMQ 延迟消息是怎么实现的？**
"基于时间轮（Time Wheel）。消息发到 Broker 后，先放到内部 Schedule Topic，时间轮定时扫描，到期后转移到真正的 Topic，消费者才可见。4.x 版本只支持预设的 18 个延迟级别；5.x 支持任意时间延迟。"

**Q：RocketMQ 和 Kafka 的事务有什么区别？**
"Kafka 事务解决的是 Kafka 内部（Consume → Process → Produce）的原子性，适合流处理；RocketMQ 事务消息解决的是**业务 DB 写入和 MQ 消息发送**之间的原子性，这是分布式事务的轻量级方案，适合电商订单、支付等场景。"

**Q：顺序消息怎么实现？有什么代价？**
"两点：1) 生产者用 MessageQueueSelector，让相同业务 Key 的消息到同一个队列；2) 消费者用 MessageListenerOrderly，串行消费同一队列。代价是吞吐下降，因为队列内是串行消费的。"

## 本文总结

RocketMQ 的三大高级特性：

1. **延迟消息**：基于时间轮，预设 18 个级别（5.x 支持任意时间），适合订单超时场景
2. **事务消息**：半消息 + 本地事务 + 回查，保证 DB 和 MQ 的原子性（最终一致性）
3. **顺序消息**：相同 Key → 同一队列 + 串行消费，保证有序性（代价是吞吐下降）

和 Kafka 的核心区别：RocketMQ 是为**业务消息**设计的（事务、延迟、顺序），Kafka 是为**日志流**设计的（极致吞吐）。

**面试高频**：事务消息流程（半消息 → 本地事务 → Commit/Rollback → 回查）、延迟消息实现（时间轮）、顺序消息实现。
...
|---|--------|------|
| 1 | 消息队列核心概念.md | ~9000 |
| 2 | Kafka架构与原理.md | ~9000 |
| 3 | Kafka消费语义.md | ~9000 |
| 4 | Kafka高可靠与高吞吐.md | ~9000 |
| 5 | RabbitMQ架构与交换机.md | ~9000 |
| 6 | RabbitMQ消息可靠性.md | ~9000 |
| 7 | RocketMQ架构与核心概念.md | ~9000 |
| 8 | RocketMQ消息模型与高可靠.md | ~9000 |

**三个 MQ 的选型建议**：
- 日志、流处理、大数据管道 → **Kafka**
- 业务消息、低延迟、灵活路由 → **RabbitMQ**
- 电商、金融、需要事务/延迟/顺序消息 → **RocketMQ**（首选）
