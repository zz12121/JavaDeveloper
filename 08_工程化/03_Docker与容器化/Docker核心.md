...
Docker 最深刻的设计是**分层镜像 + 写时复制（Copy-on-Write）**：

- **分层**：让镜像分发高效（只传输变化的层）
- **写时复制**：让多个容器共享同一个镜像，但各自有独立的可写层

这和设计模式中的「原型模式」异曲同工——不是复制整个对象，而是共享不变部分，只复制变化部分。
...
|-----------|----------|
| 镜像（只读） | 类定义（模板） |
| 容器（可写层） | 对象实例（基于类创建，各有各的状态） |
| 分层存储 | 类继承（父类不变，子类追加） |
| 多阶段构建 | 编译时依赖 vs 运行时依赖分离 |

**面试高频考点**：
1. 容器和虚拟机的区别 —— 共享内核 vs 硬件虚拟化，重点讲启动速度和资源占用
2. 镜像分层的好处 —— 共享基础层、构建缓存、增量传输
3. 不指定 `USER` 的安全风险 —— 容器内 root = 宿主机 root（如果挂载了宿主机目录）
4. 多阶段构建的作用 —— 分离构建环境和运行环境，减小镜像体积

*Docker 是目前软件工程中最成功的「环境标准化」工具。理解它，不只是为了「会写 Dockerfile」，而是理解「如何让软件在任何环境可靠运行」——这是工程能力的核心。*
