...
**Q：Netty 的线程模型？**
"Netty 采用主从 Reactor 多线程模型。BossGroup（Main Reactor）负责接受连接，默认 1 个线程；WorkerGroup（Sub Reactor）负责读写，默认 CPU 核心数 ×2 个线程。每个连接绑定一个 EventLoop，同一个连接的所有事件都由同一个 EventLoop 处理，天然线程安全，不需要同步。"

**Q：什么是粘包/拆包？怎么解决？**
"TCP 是流式协议，没有消息边界。发送方两次 send，接收方可能收到一条（粘包）或一条被截断（拆包）。Netty 提供了多种拆包器：FixedLength、LineBased、DelimiterBased、LengthField。生产环境推荐 LengthFieldBasedFrameDecoder，消息头指定消息体长度，最可靠。"

**Q：ByteBuf 和 ByteBuffer 的区别？**
"三个关键区别：① ByteBuf 有读写两个指针，不需要手动 flip；② 引用计数管理内存，明确知道什么时候可以释放；③ 支持池化复用，减少 GC 压力。生产环境建议用 PooledDirectByteBuf。"

**Q：Netty 的零拷贝？**
"Netty 的零拷贝不是 OS 层面的零拷贝，而是在用户态减少数据拷贝。三种方式：① CompositeByteBuf 合并多个 Buffer 但不拷贝数据；② FileRegion 文件传输调用 sendfile() 跳过用户空间；③ ByteBuf.slice() 切片共享底层内存。"

**Q：为什么 Netty 比原生 NIO 好？**
"原生 NIO 要自己处理半包粘包、空闲检测、优雅关闭、ByteBuffer flip 等问题，代码复杂且容易出错。Netty 提供了主从 Reactor 线程模型、ByteBuf 引用计数、ChannelPipeline 责任链、丰富的编解码器，还修复了 JDK 的空轮询 bug。生产环境几乎不用原生 NIO，Netty 是标准选择。"

**Q：EventLoop 为什么不用锁？**
"因为一个连接绑定一个 EventLoop，这个连接的所有事件（读、写、关闭）都在同一个 EventLoop 线程里串行执行，不存在并发访问，所以不需要锁。如果需要在 EventLoop 线程执行任务，用 `channel.eventLoop().execute()` 提交到 TaskQueue。"

---

## 本文总结

**Netty** 是 Java 高性能网络编程的事实标准，核心要点：

- **Reactor 模型**：主从 Reactor 多线程，BossGroup 接受连接，WorkerGroup 处理读写
- **EventLoop**：一个线程 + 一个 Selector + 一个 TaskQueue，连接绑定 EventLoop，天然无锁
- **ChannelPipeline**：责任链模式，Inbound/Outbound 双向处理器链，灵活组装
- **粘包拆包**：TCP 流式协议的经典问题，推荐 LengthFieldBasedFrameDecoder
- **ByteBuf**：双指针 + 引用计数 + 池化，比 ByteBuffer 好用得多
- **零拷贝**：CompositeByteBuf / FileRegion / slice，减少用户态数据拷贝
- **心跳机制**：IdleStateHandler + 自定义心跳消息，检测半开连接
- **空轮询 Bug**：JDK NIO 的已知问题，Netty 通过重建 Selector 解决

**高频面试考点**：Reactor 模型、EventLoop 原理、粘包拆包、ByteBuf vs ByteBuffer、零拷贝。

---

**关联文档**：
- [[常见中间件综合]]（ZooKeeper/雪花算法/中间件对比总览）
- [[Dubbo核心原理]]（Dubbo 底层通信依赖 Netty）
- [[IO与NIO]]（Java IO 模型基础）
