...
**关联文档**：
- [[06_数据库/01_MySQL架构/MySQL体系结构]]（关系型 vs 文档型架构对比）
- [[06_数据库/06_PostgreSQL/索引原理与优化]]（MongoDB 索引 vs B+树索引）
- [[06_数据库/06_PostgreSQL/MVCC与事务处理]]（WiredTiger MVCC vs InnoDB MVCC）
- [[06_数据库/06_PostgreSQL/SQL优化与执行计划]]（MongoDB explain vs EXPLAIN）
- Elasticsearch核心概念与架构（待创建）（MongoDB 主存储 + ES 搜索引擎组合架构）
