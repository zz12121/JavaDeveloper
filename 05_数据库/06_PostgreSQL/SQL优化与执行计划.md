...
**Q1：PostgreSQL 的 EXPLAIN 和 MySQL 的 EXPLAIN 有什么不同？**

> PostgreSQL 的 EXPLAIN 输出是树形结构（更直观），MySQL 是表格（type/key/rows/Extra）。PostgreSQL 的 EXPLAIN ANALYZE 可以真正执行查询，看到实际执行时间和行数，非常有用。MySQL 的 EXPLAIN 也可以显示实际执行（MySQL 8.0+），但需要特定格式。

**Q2：怎么分析 PostgreSQL 的慢查询？**

> 首先安装 pg_stat_statements 扩展，它可以统计所有查询的执行时间、调用次数、返回行数。用 `SELECT * FROM pg_stat_statements ORDER BY total_time DESC` 找到最慢的查询，然后用 `EXPLAIN ANALYZE` 分析执行计划，看是哪一步慢（是 Seq Scan 还是 Sort 用了磁盘）。

**Q3：统计信息过期会导致什么问题？怎么解决？**

> 统计信息过期会导致优化器选错执行计划（如把 Hash Join 选成 Nested Loop），查询突然变慢。解决：手动执行 `ANALYZE my_table` 更新统计信息，或者配置 AutoVacuum 更积极地触发 ANALYZE（`autovacuum_analyze_scale_factor = 0.02`）。

**Q4：PostgreSQL 的分页为什么不要用 OFFSET？用什么替代？**

> OFFSET 越大，查询越慢（需要扫描并跳过 N 行）。替代方案：游标分页（Keyset Pagination），用 `WHERE created_at < 上一页最后一行的时间 ORDER BY created_at DESC LIMIT 10`，前提是排序键有索引且唯一。

**Q5：CTE（WITH 子句）在 PostgreSQL 里是怎么优化的？**

> PostgreSQL 11 及之前，CTE 会被物化（相当于临时表），优化器无法把 CTE 内联到主查询。PostgreSQL 12+ 默认内联（不物化），和优化器融合成一条执行计划，性能更好。但如果 CTE 被多次引用，还是会自动物化（避免重复计算）。
...
| 核心概念 | 要点 |
|----------|------|
| **EXPLAIN** | 树形结构，EXPLAIN ANALYZE 看实际执行时间（非常重要） |
| **pg_stat_statements** | 慢查询分析神器，需要安装扩展 |
| **统计信息** | 必须定期 ANALYZE，否则执行计划选择错误 |
| **Index Only Scan** | 需要 VM 干净（定期 VACUUM），否则退化成 Index Scan |
| **分页优化** | 用游标分页（Keyset Pagination），不用 OFFSET |
| **CTE** | PG 12+ 默认内联（更快），多次引用自动物化 |

**PostgreSQL SQL 优化的核心**：保持统计信息新鲜（ANALYZE），用 EXPLAIN ANALYZE 分析慢查询，复杂查询用 CTE 或窗口函数。
