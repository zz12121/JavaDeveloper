...
MyBatis 动态 SQL 的本质是 **XML 模板 + OGNL 表达式引擎**。

- 加载时：把 XML 标签解析成 `SqlNode` 树（类似 AST）
- 运行时：根据入参（OGNL 表达式求值）决定哪些 `SqlNode` 贡献 SQL 片段
- 最终：拼出完整 SQL → 解析 `#{}`/`${}` → 生成 `BoundSql`

**`#{}` vs `${}` 是最经典的面试考点**：`#{}` 是预编译（`?` 占位符，安全），`${}` 是字符串替换（有注入风险，只能用于表名/字段名等场景）。

**性能提示**：如果 SQL 没有动态标签，MyBatis 会用 `RawSqlSource`（不需要每次解析 SqlNode 树），性能更好——所以能用静态 SQL 就不要用动态 SQL。
